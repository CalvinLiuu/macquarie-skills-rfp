...
2. Sync the `true_source` folder first.
3. Convert the mirrored documents into structured Markdown with `build_markdown_knowledge_base`.
4. Ensure the output creates:
   ...
   - one document-type pack per specialist lane such as `rfp`, `macquarie_current_state`, `macquarie_implementation`, `macquarie_guideline`, and `competitor_brochure`
   - a Markdown index that makes those packs easy to navigate
5. Convert the RFP itself into Markdown with `render_rfp_markdown`.
6. Preserve question relationships by keeping related domains, related client segments, related questions, and suggested evidence packs visible.
